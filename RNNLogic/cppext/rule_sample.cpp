...
namespace cppext_rule_sample {
	...
	void work(const int id, GroundingsCountTask gct) {
		...
		for(int round = 1; round <= num_round; ++round) {  // num_samples for loop
			auto &cur = tri[rand(e) % tri.size()];
			auto h = cur.first;		// int: h
			auto t = cur.second;  // List[int: t]
			...
			ckpt();
			for(auto i : t) {
				path_cnt[0][i] = 1;		//path_cnt[length][tail] = count
				choices[0][i].push_back({-1, -1, 1});  //choices[length][tail]
			} 
			...
			for(int _ = 0; _ < MAX_LEN; ++_) {
				...
				for(auto path : path_cnt[_]) {
					auto i = path.first;  // int: t
					auto cnt = path.second;  // 1?
					for(auto edge : Ginv.a[i]) {  // List[{int: r, int: h}]
						next_cnt[edge.second] += cnt;  // number of path from t to other h
						auto &cho = next_cho[edge.second];
						long long last_c = (cho.empty() ? 0 : cho.back().c);
						cho.push_back({edge.first, i, cnt + last_c});  // (r, t (q), c)
					}
				}
			}

			ckpt();
			for(auto lp : len) {  // length for loop
				int len = lp.first, cnt = lp.second;
				if(path_cnt[len][h] == 0)
					continue;
				for(int _c = 0; _c < cnt; ++_c) { // length time  for loop
					// printf("iter _c = %d\n", _c);
					ckpt();
					vector<int> path;
					for(int p = h, l = len; l > 0; --l) {  // constructing path for loop
	...
	run(int r, map<int, int> length_time, int num_samples,  //map<int, int> -> python dict(int: int)
		...
		for(int i = 0; i < E; ++i)
			if(!G.e[i][r].empty())
				tri.push_back({i, G.e[i][r]}); // List[{int: h: List[int: t]}] find all h, t under this relation r
