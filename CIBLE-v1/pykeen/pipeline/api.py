...
def pipeline(  # noqa: C901
    *,
    ...
    #     f.write(json.dumps(evaluator_instance._ranks))
    ...
    if hasattr(model_instance, "args"):
        if model_instance.args.transe_weight == "trainable":
            _result_tracker.log_metrics(
                metrics=dict(
                    learned_transe_weight=round(model_instance.transe_weight.data[0].item(), 4),
                    learned_transe_bias=round(model_instance.bias.data[0].item(), 4),
                ),
                step=step,
            )
        
        if model_instance.args.diag_w:
            _result_tracker.log_metrics(
                metrics=dict(
                    diag_w=wandb.Histogram(model_instance.diag_w.data.cpu()),
                ),
                step=step,
            )
